StarkMatrix1 <- tbl_df(read.csv("Output_Files/Stark_Matrix_Output_27_to_34_mj_0.5.csv"))
StarkMatrix2 <- tbl_df(read.csv("Output_Files/Stark_Matrix_Output_27_to_34_mj_1.5.csv"))
StarkMatrix3 <- tbl_df(read.csv("Output_Files/Stark_Matrix_Output_27_to_34_mj_2.5.csv"))
StarkMatrix4 <- tbl_df(read.csv("Output_Files/Stark_Matrix_Output_27_to_34_mj_3.5.csv"))
...
for(k in 1:length(mj)){
  ...
  for(i in 1:size){
    ZeroEnergy.newrow <- data.frame(E0 = -1/(n1[i]- QuantumDefect(n1[i],l1[i],j1[i]))^2/2, n = n1[i], l = l1[i], j = j1[i],mj = mj[k], state = paste(n1[i],l1[i], j1[i],mj[k], sep = ','))
...
write.csv(TotalTidyEnergy, "Output_Files/Tidy_Stark_Energy_n_27_to_34_mj_0.5_to_3.5.csv", row.names = FALSE)